\documentclass[12pt,lang=cn]{elegantpaper}
...
\begin{titlepage}
    \begin{center}
        ...
        \begin{tabular}{cc}
            \sihao \kaishu{学年学期：}&
            \sihao {\dlmu[8cm]{2020——2021学年第一学期}}\vspace{12pt}\\
            \sihao \kaishu{课程名称：}&
            \sihao {\dlmu[8cm]{电子商务概论}}\vspace{12pt}\\
            \sihao \kaishu{课程代码：}&
            \sihao {\dlmu[8cm]{JYTK}}\vspace{12pt}\\
            \sihao \kaishu{任课教师：}&
            \sihao {\dlmu[8cm]{涂艳}}\vspace{12pt}\\
...
\begin{abstract}
\indent OTT服务在国内外均已成为家庭娱乐不可或缺的一部分，其商业模式具有一定的研究价值。本文将以OTT行业的开拓者与领军者Netflix为例，浅析Netflix商业模式，探究其成功之处，
并分析其可能面临的问题，对Netflix以及OTT行业的未来作展望。首先，我们从Netflix在流媒体世代的转型与起步开始，利用商业画布模板，
...
\keywords{Netflix\quad OTT服务\quad SaaS模式\quad 二次曲线理论}
\end{abstract}

\renewcommand{\abstractname}{Abstract}

\begin{abstract}
    \indent OTT services have become an indispensable part of home entertainment at home and abroad, and its business model has certain research value. This article will take Netflix, a pioneer and leader in the OTT industry, as an example, analyze the Netflix business model, explore its success, analyze the problems it may face, look forward to the future of Netflix and the OTT industry. First, we started from the transformation and start of Netflix in the streaming media generation, using the business canvas template to explore the advantages of Netflix in technology, profit model, global layout, etc., in-depth understanding of Netflix's innovations, and analysis of these innovations's effect.
Based on the analysis of Netflix's business model, we will look at other OTT service providers to understand the characteristics of these vendors and the similarities and differences with Netflix, and select competitors among them to analyze the impact of Netflix on these vendors and their confrontation Strengths and weaknesses.
Then we returned to the domestic market, explore the localization improvements made by domestic manufacturers.
Finally, we look forward to the possible development of Netflix in the foreseeable future, discuss the problems it may encounter in the future, and give possible answers to each question.
\ekeywords{Netflix\quad OTT services\quad SaaS Model\quad Quadratic curve theory}
\end{abstract}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=.6\textwidth]{brand}
    \caption{Netflix的图标变迁}
\end{figure}
...
码率，而画面更复杂（例如动作戏、追车戏、爆炸场面）的部分则分配更高的码率。不同类型的作品也会有不同的算法来进行优化。动态优化器在不降低画面质量的前提下，大大缩小了视频的体积，可为用户节省约20％的带宽。\supercite{6195531}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=.8\textwidth]{revenue}
    \caption{Netflix公司历年净收入柱状图}
\end{figure}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=.5\textwidth]{curve}
    \caption{二次曲线理论}
\end{figure}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=.45\textwidth]{recommend}
    \caption{Netflix推荐系统技术栈}
\end{figure}
...
\begin{enumerate}
    ...
    \begin{figure}[!h]
        \centering
        \includegraphics[width=.5\textwidth]{trending}
        \caption{现正热播}
    \end{figure}
    ...
    \begin{figure}[!h]
        \centering
        \includegraphics[width=.5\textwidth]{topn}
        \caption{榜单}
    \end{figure}
    ...
    \begin{figure}[!h]
        \centering
        \includegraphics[width=.5\textwidth]{similar}
        \caption{类似影片}
    \end{figure}
    ...
    \begin{figure}[!h]
        \centering
        \includegraphics[width=.5\textwidth]{continue}
        \caption{继续观看}
    \end{figure}
\end{enumerate}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=.43\textwidth]{content}
...
对Netflix造成的冲击。由\textbf{图\ref{pic}}可以看出，自制内容增长迅速，其余内容增长放缓，自制内容占媒体库比例逐年降低。
...
《硅谷》等热播剧也拥有了稳定的用户群，并且HBO的线性排播模式使得其成本较Netflix要低。有着电子租赁经验（iTunes）的Apple也推出了Apple TV+，不过由于平台的封闭性用户数量无法与Netflix相竞争。
对Netflix最具威胁的是老牌内容制作商Disney+，手握大量版权的它不仅打造出了爆款《曼达洛人》，还收回了与Netflix共同制作的漫威剧集版权，影视内容非常丰富。
...
Netflix几乎全部收入来自会员费，而爱奇艺有一大部分收入来自广告服务。不过随着我国经济的快速发展，消费者的付费意愿在增强，爱奇艺等国内流媒体也在内容制作、版权采购等方面发力，增长潜力巨大。
而且国内的环境对爱奇艺等厂商较为友好，竞争压力较小。
\begin{figure}[!h]
    \centering
    \includegraphics[width=.6\textwidth]{locked}
    \caption{国外强有力的竞争厂商尚未进入中国市场}
\end{figure}
...
% 生成参考文献
\phantomsection
\addcontentsline{toc}{section}{参考文献} %向目录中添加条目
\bibliography{wpref}
